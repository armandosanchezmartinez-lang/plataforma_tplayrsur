...
<main class="main">
    ...
    <?php if (!empty($cumplimiento_inferior_labels)): ?>
    <div class="evo-card hierarchy-performance-card">
        ...
        <div class="hierarchy-performance-wrap">
            <canvas id="cCumplimientoInferior"></canvas>
        </div>
    </div>
    <?php endif; ?>
...
// --- CUMPLIMIENTO NIVEL INFERIOR VS META ---
const cumplimientoInfLabels = <?= json_encode($cumplimiento_inferior_labels) ?>;
const cumplimientoInfReal = <?= json_encode($cumplimiento_inferior_real) ?>;
const cumplimientoInfMeta = <?= json_encode($cumplimiento_inferior_meta) ?>;
const cumplimientoInfPct = <?= json_encode($cumplimiento_inferior_pct) ?>;

if (document.getElementById('cCumplimientoInferior') && cumplimientoInfLabels.length > 0) {
    new Chart(document.getElementById('cCumplimientoInferior'), {
        type: 'bar',
        data: {
            labels: cumplimientoInfLabels,
            datasets: [
                {
                    label: 'Real',
                    data: cumplimientoInfReal,
                ...
                {
                    label: 'Meta',
                    data: cumplimientoInfMeta,
                    backgroundColor: txBrandColors.azulElectrico,
                    borderRadius: 8,
                    barPercentage: 0.72,
                    categoryPercentage: 0.72
                }
        ...
        options: {
            ...
            plugins: {
                ...
                datalabels: {
                    ...
                    formatter: (value, ctx) => {
                        ...
                        const pct = cumplimientoInfPct[ctx.dataIndex] || 0;
                ...
                tooltip: {
                    callbacks: {
                        afterBody: (items) => {
                            ...
                            return `Cumplimiento: ${(cumplimientoInfPct[idx] || 0).toFixed(1)}%`;
                        }
                    }
                }
            },
            scales: {
                ...
                y: {
                    grid: { display: false },
                    ticks: { font: { size: 11, weight: 'bold' }, color: '#1a2540' }
                }
            }
        },
