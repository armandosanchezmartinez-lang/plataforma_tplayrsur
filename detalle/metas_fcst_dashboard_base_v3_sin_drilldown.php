...
function dist_norm_sql($col='distrito'){
    return "CASE
        WHEN UPPER(REPLACE(REPLACE(TRIM($col), '/', ' '), '-', ' ')) REGEXP 'COATZA[[:space:]]+MINA'
        THEN 'COATZA / MINA'
        ELSE UPPER(TRIM($col))
    END";
}
...
/* Detalle semanal por distrito optimizado con GROUP BY y distrito normalizado */
$detalle_semanal = [];
foreach($distritos as $d){
    $d = norm_dist($d);
    $detalle_semanal[$d] = [];
...
<thead>
<tr><th class="district-sticky" rowspan="2">Distrito</th><?php for($s=1;$s<=$semana;$s++): ?><th class="week-group" colspan="5">SEM <?=h($s)?></th><?php endfor; ?></tr>
