...
foreach ($vendedores as $vend) {
    ...
    if ($reai_total == 0 && $dif_tmp < 0 && $prod_3m_tmp < .70) {
...
<head>
    ...
    <style>
        ...
        body.page-reai .table-card th:nth-child(1), body.page-reai .table-card td:nth-child(1){width:23%;}
        body.page-reai .table-card th:nth-child(2), body.page-reai .table-card td:nth-child(2){width:6%;}
        body.page-reai .table-card th:nth-child(3), body.page-reai .table-card td:nth-child(3){width:6%;}
        body.page-reai .table-card th:nth-child(4), body.page-reai .table-card td:nth-child(4){width:6%;}
        body.page-reai .table-card th:nth-child(5), body.page-reai .table-card td:nth-child(5){width:8%;}
        body.page-reai .table-card th:nth-child(6), body.page-reai .table-card td:nth-child(6){width:7%;}
        ...
        body.page-reai .table-card th:nth-child(9), body.page-reai .table-card td:nth-child(9){width:8%;}
        body.page-reai .table-card th:nth-child(10), body.page-reai .table-card td:nth-child(10){width:15%;}
    ...
    <div class="table-card">
        <table>
            ...
            <?php foreach ($vendedores as $vend):
                ...
                // Regla de acción sugerida con PROD. 3M:
                // - Nunca sugerir I; Incidencia es sólo documental/informativa.
                // - Si va igual/mejor que el período anterior (dif >= 0), OK.
                // - Si PROD. 3M es verde (>= .70), OK aunque el período actual venga bajo.
                // - PROD Día amarilla + PROD. 3M amarilla + caída = Aplicar R.
                // - PROD Día roja + PROD. 3M amarilla + caída = Aplicar E.
                // - PROD Día roja + PROD. 3M roja + caída fuerte/cero producción = Aplicar A.
                $caida_pronunciada = ($pct <= -50 || $dif <= -2);
                $caida_leve        = ($dif < 0 && !$caida_pronunciada);

                if ($reai_total > 0) {
                    $estatus_txt = 'En seguimiento'; $estatus_cls = 'status-seg';
                } elseif ($dif >= 0 || $prod >= .70 || $prod_3m >= .70) {
                    $estatus_txt = 'OK'; $estatus_cls = 'status-ok';
                } elseif ($prod < .40 && $prod_3m < .40 && ($caida_pronunciada || $inst_act == 0)) {
                    $estatus_txt = 'Aplicar A'; $estatus_cls = 'status-risk';
                } elseif ($prod < .40 && $prod_3m < .70 && $dif < 0) {
                    $estatus_txt = 'Aplicar E'; $estatus_cls = 'status-risk';
                } elseif ($prod >= .40 && $prod < .70 && $prod_3m < .70 && $dif < 0) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const colActual = <?= $periodo === 'mensual' ? 3 : 3 ?>; // columna JUN/SEM actual, índice 0-based
