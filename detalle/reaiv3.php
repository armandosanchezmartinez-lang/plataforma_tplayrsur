...
foreach ($vendedores as $vend) {
    ...
    if ($inst_actual == 0 && $reai_total == 0) $total_riesgo++;
    elseif ($inst_actual < $inst_base && $reai_total == 0) $total_riesgo++;
}
?>
<!DOCTYPE html>
<html lang="es">
    ...
    <div class="table-card">
        <table>
            ...
            <?php foreach ($vendedores as $vend):
                ...
                if ($inst_act == 0 && $reai_total == 0) {
                    $estatus_txt = 'Sin prod / Sin REAI'; $estatus_cls = 'status-risk';
                } elseif ($inst_act < $inst_base && $reai_total == 0) {
                    $estatus_txt = 'Requiere R'; $estatus_cls = 'status-alert';
                } elseif ($reai_total > 0) {
            ...
            <tr data-nombre="<?= strtolower(h($nombre)) ?>">
                <td class="left">
                    <div style="font-weight:600;">
                        <a href="detalle_vendedor.php?tgs=<?= urlencode($tgs) ?>" style="color:var(--blue);text-decoration:none;font-weight:700;" title="Ver detalle del vendedor">
                            <?= h($nombre) ?>
                        </a>
                    </div>
                    <div class="sub-text"><?= h($tgs) ?></div>
                </td>
                <td><span style="font-weight:800;"><?= $antig ?></span> <span class="sub-text">m</span></td>
                <td><span style="font-weight:900;"><?= fmt_num($inst_base) ?></span></td>
                <td><span style="font-weight:900;"><?= fmt_num($inst_act) ?></span></td>
                <td><span class="<?= $pct_cls ?>"><?= $dif >= 0 ? '+' : '' ?><?= fmt_num($dif) ?> / <?= $pct >= 0 ? '+' : '' ?><?= fmt_num($pct) ?>%</span></td>
                <td><span class="prod-pill <?= $prod_cls ?>"><?= fmt_prod($prod) ?></span></td>
                <td><span class="status-pill <?= $estatus_cls ?>"><?= h($estatus_txt) ?></span></td>
                <td class="sep">
                    <?php
                    $asuntos_map = [
