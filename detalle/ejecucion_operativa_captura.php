...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($bloqueado) {
        ...
        if ($accion_form === 'enviar' && !empty($subordinados_tmp) && $meta_distribuida_tmp < $meta_responsable_tmp) {
            $mensaje = "No se puede enviar. La meta distribuida ($meta_distribuida_tmp) es menor a la meta del responsable ($meta_responsable_tmp).";
            $tipo_mensaje = 'error';
        } else {
            mysqli_begin_transaction($conexion);
            try {
                $observacion_fcst = 'Captura Ejecución Operativa';
                $archivo_origen = 'ejecucion_operativa_web';
                ...
                mysqli_stmt_bind_param(
                    ...
                    $nombre_responsable, $puesto_responsable, $distrito, $nivel_ejecucion,
                ...
                mysqli_stmt_bind_param(
                    ...
                    $impulso_semana_anterior, $resto_semana_anterior, $competencia,
                    $acciones_clave, $necesidades_apoyo, $estatus_fcst, $cerrado_en,
                    $cerrado_por, $usuario
                );
                if (!mysqli_stmt_execute($stmt)) throw new Exception(mysqli_error($conexion));
                mysqli_stmt_close($stmt);
