...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if($id_ejecucion<=0){ $mensaje='Primero debes crear el plan operativo de la semana para poder documentar acompañamientos.'; $tipo_mensaje='error'; }
        ...
        if($tipo_colaborador==='' || $nombre_colaborador===''){ $mensaje='Captura al menos el tipo de colaborador y el nombre del colaborador acompañado.'; $tipo_mensaje='error'; }
            ...
            $sql="INSERT INTO ejecucion_operativa_acompanamientos (id_ejecucion,id_accion,anio,semana,id_posicion_jefe,nombre_jefe,nivel_jefe,distrito,fecha_hora,tipo_colaborador,id_posicion_colaborador,numero_talento_colaborador,nombre_colaborador,tipo_acompanamiento,hallazgos_principales,compromisos,usuario_captura) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            $stmt=mysqli_prepare($conexion,$sql);
            if(!$stmt){ $mensaje='Error preparando guardado: '.mysqli_error($conexion); $tipo_mensaje='error'; }
            else{
                mysqli_stmt_bind_param($stmt,"iiiisssssssssssss",$id_ejecucion,$id_accion,$anio_actual,$semana_actual,$id_posicion,$nombre_jefe,$nivel_jefe,$distrito,$fecha_hora,$tipo_colaborador,$id_posicion_colaborador,$numero_talento_colaborador,$nombre_colaborador,$tipo_acompanamiento,$hallazgos,$compromisos,$usuario);
