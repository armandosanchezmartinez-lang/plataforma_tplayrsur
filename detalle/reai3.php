...
// Fuente: metas_instalacion_semanal, sumando las semanas ISO que intersectan
// los últimos 3 meses completos del corte operativo.
$meta_3m_director_por_distrito = [];
if ($vista_nivel === 'DIRECTOR DISTRITAL' && table_exists($conexion, 'metas_instalacion_semanal')) {
    $semanas_3m = semanas_iso_en_rango_reai($fecha_3m_inicio, $fecha_3m_fin);
    if (!empty($semanas_3m)) {
        $conds = [];
        foreach ($semanas_3m as $sw) {
            $conds[] = '(anio = '.(int)$sw['anio'].' AND semana = '.(int)$sw['semana'].')';
        }
        $sql_meta_3m_dd = "SELECT distrito, SUM(meta) AS meta_3m FROM metas_instalacion_semanal WHERE " . implode(' OR ', $conds) . " GROUP BY distrito";
...
if (!empty($vendedores)) {
    ...
    foreach ($vendedores as $row) {
        ...
        // metas_instalacion_semanal acumulada de los últimos 3 meses completos.
...
<head>
    ...
    <title>REAI v2.4 — TOTALXPEDIENT</title>
