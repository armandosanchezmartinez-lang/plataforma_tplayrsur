...
<section class="topbar">
    ...
    <div class="week-nav">
        ...
        ?>
            ...
            <form class="month-day-filter compact" method="get">
                <?php foreach ($_GET as $k => $v): ?>
                    <?php if (!in_array($k, ['dia_inicio','dia_fin'], true)): ?>
                        <input type="hidden" name="<?= h($k) ?>" value="<?= h($v) ?>">
                    <?php endif; ?>
                <?php endforeach; ?>

                <label>Rango:</label>
                <input type="number" name="dia_inicio" min="1" max="<?= h($ultimo_dia_actual) ?>" value="<?= h($dia_inicio_mensual) ?>">
                <span>al</span>
                <input type="number" name="dia_fin" min="1" max="<?= h($ultimo_dia_actual) ?>" value="<?= h($dia_fin_mensual) ?>">
                <button type="submit">Aplicar</button>

                <a class="quick-range" href="?<?= qs(array_merge($_GET, ['periodo'=>'mensual','dia_inicio'=>1,'dia_fin'=>$dia_default_mensual])) ?>">MTD día vencido</a>
                <a class="quick-range" href="?<?= qs(array_merge($_GET, ['periodo'=>'mensual','dia_inicio'=>1,'dia_fin'=>$ultimo_dia_actual])) ?>">Mes completo</a>

                <span class="hint"><?= h($meses_es[$mes_base]) ?> <?= h($dia_inicio_base) ?>-<?= h($dia_fin_base) ?> vs <?= h($meses_es[$mes_actual]) ?> <?= h($dia_inicio_actual) ?>-<?= h($dia_fin_actual) ?></span>
            </form>
        <?php endif; ?>
    </div>
</section>
