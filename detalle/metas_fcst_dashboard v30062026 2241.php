...
function coach_tokens_where($col, $nombre, &$params, &$types) {
    $nombre = trim((string)$nombre);
    if ($nombre === '') return "1=0";

    $stop = ['DE','DEL','LA','LAS','LOS','Y'];
    $tokens = preg_split('/\s+/', strtoupper(strtr($nombre, [
    ...
    if (empty($tokens)) return "UPPER(TRIM($col)) = UPPER(TRIM(?))";

    $parts = [];
    foreach ($tokens as $t) {
        $parts[] = "UPPER($col) LIKE ?";
        $params[] = '%' . $t . '%';
        $types .= 's';
    }
...
function cargar_grupos($conexion, $anio, $semana, $scope, $usuario_hc){
    ...
    $distrito_jefe = norm_dist($usuario_hc['distrito'] ?? '');
    $dn_hc = dist_norm_sql('distrito');

    /*
     * Blindaje jerárquico por distrito:
     * Para Director Distrital y Líder, se exige posicion_lr + distrito normalizado.
     * Así evitamos que se cuele un responsable de otro distrito por registros históricos
     * o semanas sin HC exacto.
     */
    if($scope['nivel']==='REGIONAL'){
        ...
        if($id_jefe!==''){
            if($distrito_jefe!==''){
                $stmt=mysqli_prepare($conexion,"SELECT id_posicion,posicion_lr,numero_talento_gs,nombre_colaborador,posicion,distrito FROM hc WHERE semana=? AND anio=? AND posicion IN ($puestos) AND posicion_lr=? AND $dn_hc=? AND numero_talento_gs NOT LIKE '%VACANTE%' ORDER BY nombre_colaborador");
                if($stmt){ mysqli_stmt_bind_param($stmt,"iiss",$semana,$anio,$id_jefe,$distrito_jefe); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); while($r=mysqli_fetch_assoc($res))$grupos[]=$r; mysqli_stmt_close($stmt); }
            } else {
    ...
    if(!$grupos){
        if($scope['nivel']==='REGIONAL'){
            ...
            if($distrito_jefe!==''){
                $dn_sub = dist_norm_sql('distrito');
                $dn_h = dist_norm_sql('h.distrito');
                $sql = "SELECT h.* FROM hc h
                        INNER JOIN (
                            SELECT id_posicion, MAX(CONCAT(anio,LPAD(semana,2,'0'))) periodo
                            FROM hc
                            WHERE posicion IN ($puestos)
                              AND posicion_lr=?
                              AND $dn_sub=?
                              AND numero_talento_gs NOT LIKE '%VACANTE%'
                            GROUP BY id_posicion
                        ) u ON h.id_posicion=u.id_posicion AND CONCAT(h.anio,LPAD(h.semana,2,'0'))=u.periodo
                        WHERE h.posicion IN ($puestos)
                          AND h.posicion_lr=?
                          AND $dn_h=?
                        ORDER BY h.nombre_colaborador";
                $stmt=mysqli_prepare($conexion,$sql);
                if($stmt){ mysqli_stmt_bind_param($stmt,"ssss",$id_jefe,$distrito_jefe,$id_jefe,$distrito_jefe); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); while($r=mysqli_fetch_assoc($res))$grupos[]=$r; mysqli_stmt_close($stmt); }
            } else {
    ...
    $seen=[]; $out=[];
    foreach($grupos as $g){
        $k=(string)($g['id_posicion'] ?? '');
        if($k==='' || isset($seen[$k])) continue;
        $g['distrito']=norm_dist($g['distrito'] ?? '');

        // Último filtro de seguridad: en vistas no regionales, no mostrar subordinados de otro distrito.
        if($scope['nivel']!=='REGIONAL' && $scope['nivel']!=='COACH' && $distrito_jefe!=='' && $g['distrito']!==$distrito_jefe) continue;
...
function real_por_grupo($conexion,$anio,$semana,$scope,$g){
    $id=(string)($g['numero_talento_gs'] ?? '');
    $nombre=(string)($g['nombre_colaborador'] ?? '');
    $dist=norm_dist($g['distrito'] ?? '');
    $v=0;

    if(($scope['target'] ?? '')==='DIRECTOR_DISTRITAL'){
        $stmt=mysqli_prepare($conexion,"SELECT COUNT(cuenta) total FROM instalaciones WHERE YEAR(fecha)=? AND WEEK(fecha,1)=? AND ".dist_norm_sql('distrito')."=? AND origen_prospecto <> '-'");
        if($stmt){
            mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$dist);
            mysqli_stmt_execute($stmt);
            $res=mysqli_stmt_get_result($stmt);
            if($r=mysqli_fetch_assoc($res)) $v=(int)$r['total'];
            mysqli_stmt_close($stmt);
        }
        return $v;
    }

    if(($scope['target'] ?? '')==='LIDER_VENTAS'){
        ...
        if($stmt){
            mysqli_stmt_bind_param($stmt,"iiss",$anio,$semana,$id,$nombre);
            mysqli_stmt_execute($stmt);
            $res=mysqli_stmt_get_result($stmt);
            if($r=mysqli_fetch_assoc($res)) $v=(int)$r['total'];
            mysqli_stmt_close($stmt);
    ...
    if(($scope['target'] ?? '')==='COACH_VENTAS'){
        /*
         * FIX nivel COACH:
         * instalaciones.folio_coach viene NULL en varios casos y el nombre del coach puede venir invertido.
         * Ejemplo:
         * HC: ARCOS GARCIA ALEJANDRO
         * INS: ALEJANDRO ARCOS GARCIA
         * Por eso usamos folio si existe y, como respaldo, cruce por tokens del nombre.
         */
        $params = [];
        $types = '';
        $token_where = coach_tokens_where('coach', $nombre, $params, $types);

        $sql = "SELECT COUNT(cuenta) total
                FROM instalaciones
                WHERE YEAR(fecha)=?
                  AND WEEK(fecha,1)=?
        ...
        $stmt=mysqli_prepare($conexion,$sql);
        if($stmt){
            $bind_types = "iis" . $types;
            $bind_params = array_merge([$anio,$semana,$id], $params);
...
function plan_operativo_por_id($conexion,$anio,$semana,$id_pos,$meta,$scope){
    $out=['id_ejecucion'=>0,'estatus'=>'SIN PLAN','distribuida'=>0,'pct_dist'=>null,'palancas'=>0,'acciones'=>0,'plan_cls'=>'rojo','dist_cls'=>'rojo','palancas_cls'=>'rojo','acciones_cls'=>'rojo'];

    $stmt=mysqli_prepare($conexion,"SELECT id, estatus FROM ejecucion_operativa WHERE anio=? AND semana=? AND id_posicion=? LIMIT 1");
    if($stmt){ mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$id_pos); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); if($r=mysqli_fetch_assoc($res)){ $out['id_ejecucion']=(int)$r['id']; $out['estatus']=$r['estatus'] ?: 'BORRADOR'; } mysqli_stmt_close($stmt); }

    /*
     * Distribución mostrada en la tabla:
     * - En vista regional, cada fila es un Director Distrital, por eso mostramos lo que ese Director ya distribuyó a sus líderes.
     * - En vistas Director/Líder/Coach, cada fila es un subordinado, por eso mostramos la meta que recibió desde su superior.
     * Esto corrige casos como PÁRAMO: su meta asignada debe verse aunque él aún no haya distribuido a su propia estructura.
     */
    if(($scope['target'] ?? '') === 'DIRECTOR_DISTRITAL'){
        $stmt=mysqli_prepare($conexion,"SELECT COALESCE(SUM(meta_asignada),0) total FROM ejecucion_operativa_metas WHERE anio=? AND semana=? AND id_superior=?");
        if($stmt){ mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$id_pos); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); if($r=mysqli_fetch_assoc($res)) $out['distribuida']=(int)$r['total']; mysqli_stmt_close($stmt); }
    } else {
        $stmt=mysqli_prepare($conexion,"SELECT COALESCE(SUM(meta_asignada),0) total FROM ejecucion_operativa_metas WHERE anio=? AND semana=? AND id_subordinado=?");
        if($stmt){ mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$id_pos); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); if($r=mysqli_fetch_assoc($res)) $out['distribuida']=(int)$r['total']; mysqli_stmt_close($stmt); }
    }

    if($out['id_ejecucion']>0){
        $stmt=mysqli_prepare($conexion,"SELECT COUNT(DISTINCT id_palanca) total FROM ejecucion_operativa_plan_palancas WHERE id_ejecucion=?");
        if($stmt){ mysqli_stmt_bind_param($stmt,"i",$out['id_ejecucion']); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); if($r=mysqli_fetch_assoc($res)) $out['palancas']=(int)$r['total']; mysqli_stmt_close($stmt); }

        $stmt=mysqli_prepare($conexion,"SELECT COUNT(*) total FROM ejecucion_operativa_acciones WHERE id_ejecucion=?");
        if($stmt){ mysqli_stmt_bind_param($stmt,"i",$out['id_ejecucion']); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt); if($r=mysqli_fetch_assoc($res)) $out['acciones']=(int)$r['total']; mysqli_stmt_close($stmt); }
    }

    $out['pct_dist']=div_pct($out['distribuida'],$meta);

    if($out['estatus']==='ENVIADO' || $out['estatus']==='CERRADO') $out['plan_cls']='verde';
    elseif($out['estatus']==='BORRADOR') $out['plan_cls']='amarillo';

    if($out['pct_dist']!==null && $out['pct_dist']>=100) $out['dist_cls']='verde';
    elseif($out['pct_dist']!==null && $out['pct_dist']>0) $out['dist_cls']='amarillo';

    if($out['palancas']>=6) $out['palancas_cls']='verde';
    elseif($out['palancas']>=3) $out['palancas_cls']='amarillo';

    if($out['acciones']>=8) $out['acciones_cls']='verde';
    elseif($out['acciones']>=5) $out['acciones_cls']='amarillo';
...
foreach($grupos as $g){
    ...
    $plan=plan_operativo_por_id($conexion,$anio,$semana,$idg,$meta,$scope);
    ...
    $rows[]=['id_posicion'=>$idg,'distrito'=>$label,'meta'=>$meta,'fcst'=>$fcst,'real'=>$real,'pm'=>$pm,'pf'=>$pf,'ac'=>$ac,'cerrados'=>$comp['cerrados'],'borradores'=>$comp['borradores'],'plan_estatus'=>$plan['estatus'],'plan_cls'=>$plan['plan_cls'],'distribuida'=>$plan['distribuida'],'pct_dist'=>$plan['pct_dist'],'dist_cls'=>$plan['dist_cls'],'palancas'=>$plan['palancas'],'palancas_cls'=>$plan['palancas_cls'],'acciones'=>$plan['acciones'],'acciones_cls'=>$plan['acciones_cls'],'cm'=>cls($pm),'cf'=>cls($pf),'riesgo'=>riesgo($pm)];
}
...
<link rel="stylesheet" href="../assets/css/xpedient-v2.css?v=178">
<style>
...
<section class="card full"><div class="title"><h2>Tabla ejecutiva jerárquica · SEM <?=h($semana)?> <?=h($anio)?></h2><span>Meta · FCST · Real · Plan operativo</span></div><div class="wrap"><table><thead><tr><th><?=h($scope['label'])?></th><th>Meta</th><th>FCST</th><th>Real</th><th>% Meta</th><th>% FCST</th><th>Plan</th><th>Meta Distribuida (CAM+PDV)</th><th>% Dist</th><th>Palancas</th><th>Acciones</th><th>Plan</th></tr></thead><tbody><?php foreach($rows as $r): ?><tr><td><?=h($r['distrito'])?></td><td><?=h(fmt($r['meta']))?></td><td><?=h(fmt($r['fcst']))?></td><td><?=h(fmt($r['real']))?></td><td><span class="pill <?=h($r['cm'])?>"><?=h(fpct($r['pm']))?></span></td><td><span class="pill <?=h($r['cf'])?>"><?=h(fpct($r['pf']))?></span></td><td><span class="pill <?=h($r['plan_cls'])?>"><?=h($r['plan_estatus'])?></span></td><td><?=h(fmt($r['distribuida']))?></td><td><span class="pill <?=h($r['dist_cls'])?>"><?=h(fpct($r['pct_dist']))?></span></td><td><span class="pill <?=h($r['palancas_cls'])?>"><?=h($r['palancas'])?></span></td><td><span class="pill <?=h($r['acciones_cls'])?>"><?=h($r['acciones'])?></span></td><td><a class="week-btn" href="ejecucion_operativa_consulta.php?anio=<?=h($anio)?>&semana=<?=h($semana)?>&id_posicion=<?=urlencode($r['id_posicion'])?>">👁 Ver</a></td></tr><?php endforeach; ?>
<tr class="total-row">
<td>TOTAL</td>
...
<td>—</td>
<td><?=h(fmt(array_sum(array_column($rows,'distribuida'))))?></td>
<td><span class="pill <?=h(cls(div_pct(array_sum(array_column($rows,'distribuida')),$total_meta)))?>"><?=h(fpct(div_pct(array_sum(array_column($rows,'distribuida')),$total_meta)))?></span></td>
<td><?=h(array_sum(array_column($rows,'palancas')))?></td>
<td><?=h(array_sum(array_column($rows,'acciones')))?></td>
<td>—</td>
</tr>
