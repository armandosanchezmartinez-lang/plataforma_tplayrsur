...
foreach($distritos as $d){
    ...
    if($ac!==null && $ac<85) $alertas[]="⚠️ $d tiene baja precisión de forecast: ".fpct($ac).".";
...
for($s=1;$s<=$semana;$s++){
    ...
    $labels[]='S'.$s; $serie_meta[]=$m?round($r/$m*100,1):null; $serie_fcst[]=$f?round($r/$f*100,1):null;
}
?>
...
<section class="card full"><div class="title"><h2>Tabla ejecutiva de cumplimiento</h2><span>Meta · FCST · Real · Accuracy</span></div><div class="wrap"><table><thead><tr><th>Distrito</th><th>Meta</th><th>FCST</th><th>Real</th><th>% Meta</th><th>% FCST</th><th>Accuracy</th><th>Cerrados</th><th>Borradores</th></tr></thead><tbody><?php foreach($rows as $r): ?><tr><td><?=h($r['distrito'])?></td><td><?=h(fmt($r['meta']))?></td><td><?=h(fmt($r['fcst']))?></td><td><?=h(fmt($r['real']))?></td><td><span class="pill <?=h($r['cm'])?>"><?=h(fpct($r['pm']))?></span></td><td><span class="pill <?=h($r['cf'])?>"><?=h(fpct($r['pf']))?></span></td><td><?=h(fpct($r['ac']))?></td><td><?=h($r['cerrados'])?></td><td><?=h($r['borradores'])?></td></tr><?php endforeach; ?></tbody></table></div></section>
<section class="card full"><div class="title"><h2>Tendencia regional 2026</h2><span>META% vs FCST%</span></div><canvas id="chartRegional" class="chart"></canvas></section></div>
</main>
<script>
const labels=<?=json_encode($labels)?>, serieMeta=<?=json_encode($serie_meta,JSON_NUMERIC_CHECK)?>, serieFcst=<?=json_encode($serie_fcst,JSON_NUMERIC_CHECK)?>;
(function(){const c=document.getElementById('chartRegional');if(!c)return;const ctx=c.getContext('2d'),dpr=window.devicePixelRatio||1,r=c.getBoundingClientRect();c.width=r.width*dpr;c.height=r.height*dpr;ctx.scale(dpr,dpr);const w=r.width,h=r.height,p=38,vals=serieMeta.concat(serieFcst).filter(v=>v!==null&&!isNaN(v)),max=Math.max(140,...vals);function x(i,t){return t<=1?p:p+i*(w-p*2)/(t-1)}function y(v){return v===null?null:h-p-(v/max)*(h-p*2)}ctx.font='11px Segoe UI';ctx.strokeStyle='#e2e8f0';[0,50,90,100,120,140].forEach(v=>{let yy=y(v);ctx.beginPath();ctx.moveTo(p,yy);ctx.lineTo(w-p,yy);ctx.stroke();ctx.fillStyle='#64748b';ctx.fillText(v+'%',5,yy+4)});function line(s,color){ctx.strokeStyle=color;ctx.lineWidth=2.6;ctx.beginPath();let st=false;s.forEach((v,i)=>{let yy=y(v);if(yy===null)return;let xx=x(i,s.length);if(!st){ctx.moveTo(xx,yy);st=true}else ctx.lineTo(xx,yy)});ctx.stroke();s.forEach((v,i)=>{let yy=y(v);if(yy===null)return;let xx=x(i,s.length);ctx.beginPath();ctx.arc(xx,yy,3,0,Math.PI*2);ctx.fillStyle=color;ctx.fill()})}line(serieMeta,'#7A2BFF');line(serieFcst,'#2563eb');})();
</script></body></html>