...
function norm_dist($d){
    ...
    $d = str_replace(['-','/'], ' ', $d);
    $d = preg_replace('/\s+/', ' ', $d);
    if($d === 'COATZA MINA') return 'COATZA / MINA';
    return $d;
}
function dist_norm_sql($col='distrito'){
    return "CASE WHEN UPPER(REPLACE(REPLACE(TRIM($col),'-',' '),'/',' ')) REGEXP 'COATZA[[:space:]]+MINA' THEN 'COATZA / MINA' ELSE UPPER(TRIM($col)) END";
}
...
$distritos=[];
$dn = dist_norm_sql('distrito');
$sql="SELECT $dn distrito FROM metas_instalacion_semanal WHERE anio=? AND semana=? AND distrito IS NOT NULL AND distrito<>''
UNION SELECT $dn distrito FROM metas_forecast_semanal WHERE anio=? AND semana=? AND distrito IS NOT NULL AND distrito<>''
UNION SELECT $dn distrito FROM instalaciones WHERE YEAR(fecha)=? AND WEEK(fecha,1)=? AND distrito IS NOT NULL AND distrito<>''
ORDER BY distrito";
...
while($r=mysqli_fetch_assoc($res)) $distritos[]=norm_dist($r['distrito']);
mysqli_stmt_close($stmt);
$distritos = array_values(array_unique(array_filter($distritos)));
...
foreach($distritos as $d){
    ...
    $stmt=mysqli_prepare($conexion,"SELECT COALESCE(SUM(meta),0) total FROM metas_instalacion_semanal WHERE anio=? AND semana=? AND CASE WHEN UPPER(REPLACE(REPLACE(TRIM(distrito),'-',' '),'/',' ')) REGEXP 'COATZA[[:space:]]+MINA' THEN 'COATZA / MINA' ELSE UPPER(TRIM(distrito)) END=?");
    ...
    $stmt=mysqli_prepare($conexion,"SELECT COALESCE(SUM(forecast),0) total FROM metas_forecast_semanal WHERE anio=? AND semana=? AND CASE WHEN UPPER(REPLACE(REPLACE(TRIM(distrito),'-',' '),'/',' ')) REGEXP 'COATZA[[:space:]]+MINA' THEN 'COATZA / MINA' ELSE UPPER(TRIM(distrito)) END=?");
    mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$d); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt);
    if($r=mysqli_fetch_assoc($res))$fcst=(int)$r['total']; mysqli_stmt_close($stmt);

    $stmt=mysqli_prepare($conexion,"SELECT COUNT(cuenta) total FROM instalaciones WHERE YEAR(fecha)=? AND WEEK(fecha,1)=? AND CASE WHEN UPPER(REPLACE(REPLACE(TRIM(distrito),'-',' '),'/',' ')) REGEXP 'COATZA[[:space:]]+MINA' THEN 'COATZA / MINA' ELSE UPPER(TRIM(distrito)) END=? AND origen_prospecto <> '-'");
    mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$d); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt);
    if($r=mysqli_fetch_assoc($res))$real=(int)$r['total']; mysqli_stmt_close($stmt);

    $stmt=mysqli_prepare($conexion,"SELECT SUM(CASE WHEN c.estatus='CERRADO' THEN 1 ELSE 0 END) cerrados, SUM(CASE WHEN c.estatus='BORRADOR' THEN 1 ELSE 0 END) borradores FROM metas_fcst_compromiso_semanal c INNER JOIN metas_forecast_semanal f ON f.id=c.forecast_id WHERE c.anio=? AND c.semana=? AND CASE WHEN UPPER(REPLACE(REPLACE(TRIM(f.distrito),'-',' '),'/',' ')) REGEXP 'COATZA[[:space:]]+MINA' THEN 'COATZA / MINA' ELSE UPPER(TRIM(f.distrito)) END=?");
...
/* Detalle semanal por distrito optimizado con GROUP BY */
$detalle_semanal = [];
foreach($distritos as $d){
    $d = norm_dist($d);
    $detalle_semanal[$d] = [];
    for($s=1; $s<=$semana; $s++){
        $detalle_semanal[$d][$s] = ['meta'=>0,'fcst'=>0,'real'=>0,'pm'=>null,'pf'=>null];
    }
}

$dn = dist_norm_sql('distrito');

/* Metas oficiales por distrito/semana */
$sql="SELECT $dn distrito, semana, COALESCE(SUM(meta),0) total
      FROM metas_instalacion_semanal
      WHERE anio=? AND semana BETWEEN 1 AND ?
      GROUP BY $dn, semana";
$stmt=mysqli_prepare($conexion,$sql);
if($stmt){
    mysqli_stmt_bind_param($stmt,"ii",$anio,$semana);
    mysqli_stmt_execute($stmt);
    $res=mysqli_stmt_get_result($stmt);
    while($x=mysqli_fetch_assoc($res)){
        $dist=norm_dist($x['distrito']); $sem=(int)$x['semana'];
        if(isset($detalle_semanal[$dist][$sem])) $detalle_semanal[$dist][$sem]['meta']=(int)$x['total'];
    }
    mysqli_stmt_close($stmt);
}

/* Forecast por distrito/semana */
$sql="SELECT $dn distrito, semana, COALESCE(SUM(forecast),0) total
...
if($stmt){
    ...
    while($x=mysqli_fetch_assoc($res)){
        $dist=norm_dist($x['distrito']); $sem=(int)$x['semana'];
        if(isset($detalle_semanal[$dist][$sem])) $detalle_semanal[$dist][$sem]['fcst']=(int)$x['total'];
    }
    mysqli_stmt_close($stmt);
}

/* Instalaciones reales por distrito/semana */
$sql="SELECT $dn distrito, WEEK(fecha,1) semana, COUNT(cuenta) total
      FROM instalaciones
      WHERE YEAR(fecha)=?
        AND WEEK(fecha,1) BETWEEN 1 AND ?
        AND distrito IS NOT NULL
        AND distrito <> ''
        AND origen_prospecto <> '-'
      GROUP BY $dn, WEEK(fecha,1)";
$stmt=mysqli_prepare($conexion,$sql);
if($stmt){
    mysqli_stmt_bind_param($stmt,"ii",$anio,$semana);
    mysqli_stmt_execute($stmt);
    ...
    while($x=mysqli_fetch_assoc($res)){
        $dist=norm_dist($x['distrito']); $sem=(int)$x['semana'];
        if(isset($detalle_semanal[$dist][$sem])) $detalle_semanal[$dist][$sem]['real']=(int)$x['total'];
    }
    mysqli_stmt_close($stmt);
}

foreach($detalle_semanal as $dist=>$semanas){
    foreach($semanas as $sem=>$vals){
        $detalle_semanal[$dist][$sem]['pm'] = div_pct($vals['real'],$vals['meta']);
        $detalle_semanal[$dist][$sem]['pf'] = div_pct($vals['real'],$vals['fcst']);
    }
}
?>
...
<div class="grid"><section class="card"><div class="title"><h2>Semáforo regional por distrito</h2><span>% cumplimiento vs META</span></div><div class="heat"><?php foreach($rows as $r): ?>
<a class="dcard <?=h($r['cm'])?>" href="metas_fcst_captura.php?anio=<?=h($anio)?>&semana=<?=h($semana)?>&distrito=<?=urlencode($r['distrito'])?>" title="Ver captura FCST de <?=h($r['distrito'])?>">
    <div class="name"><?=h($r['distrito'])?></div>
    <div class="pct"><?=h(fpct($r['pm']))?></div>
    <div class="risk"><?=h($r['riesgo'])?> · Ver detalle</div>
</a>
...
<tr style="font-weight:950;background:#f8fafc;">
<td>TOTAL</td>
...
<section class="card full">
    <div class="title">
        <h2>Detalle semanal por distrito · 2026</h2>
        <span>SEM 1 a SEM <?=h($semana)?> · Meta · FCST · Real · %META · %FCST</span>
    </div>
    ...
    <div class="detalle-note">Optimizado con consultas agregadas por distrito y semana. La tabla se desplaza horizontalmente para consultar todas las semanas intermedias.</div>
</section></div>
</main>
<script>
