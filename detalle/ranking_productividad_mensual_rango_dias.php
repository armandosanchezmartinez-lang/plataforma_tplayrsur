...
$ultimo_dia_comparable = min($ultimo_dia_base, $ultimo_dia_actual);
...
$dia_inicio_mensual = max(1, min($ultimo_dia_comparable, $dia_inicio_mensual));
$dia_fin_mensual = max(1, min($ultimo_dia_comparable, $dia_fin_mensual));
...
<?php if ($periodo === 'mensual'): ?>
<form class="month-day-filter" method="get">
    ...
    <input type="number" name="dia_inicio" min="1" max="<?= h($ultimo_dia_comparable) ?>" value="<?= h($dia_inicio_mensual) ?>">
    <span>al</span>
    <input type="number" name="dia_fin" min="1" max="<?= h($ultimo_dia_comparable) ?>" value="<?= h($dia_fin_mensual) ?>">
    <button type="submit">Aplicar</button>
    <span class="hint">Compara <?= h($meses_es[$mes_base]) ?> <?= h($dia_inicio_base) ?>-<?= h($dia_fin_base) ?> vs <?= h($meses_es[$mes_actual]) ?> <?= h($dia_inicio_actual) ?>-<?= h($dia_fin_actual) ?></span>
</form>
<?php endif; ?>
