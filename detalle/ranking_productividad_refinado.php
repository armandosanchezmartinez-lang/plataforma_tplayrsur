...
.metric-prod-high{background:#22c55e !important;color:#ffffff !important;}
.metric-prod-good{background:#86efac !important;color:#14532d !important;}
.metric-prod-mid{background:#fde68a !important;color:#78350f !important;}
.metric-prod-low{background:#fdba74 !important;color:#9a3412 !important;}
.metric-prod-bad{background:#fca5a5 !important;color:#991b1b !important;}

.metric-hc-good{background:#bbf7d0 !important;color:#166534 !important;}
.metric-hc-mid{background:#fde68a !important;color:#78350f !important;}
.metric-hc-bad{background:#fecaca !important;color:#991b1b !important;}

.metric-diff-pos{background:#dbeafe !important;color:#1d4ed8 !important;}
.metric-diff-good{background:#bbf7d0 !important;color:#166534 !important;}
.metric-diff-warn{background:#fed7aa !important;color:#9a3412 !important;}
.metric-diff-bad{background:#fecaca !important;color:#991b1b !important;}
...
function applyExecutiveColors(){
 const rows=[...document.querySelectorAll('tbody tr')];
 rows.forEach(r=>{
   if((r.textContent||'').includes('TOTAL')) r.classList.add('total-row');
   const cells=r.querySelectorAll('td');
   cells.forEach(td=>{
      const t=td.textContent.trim().replace('%','').replace(',','');
      const n=parseFloat(t);
      if(isNaN(n)) return;
      const cls=(td.cellIndex||0);
      // % DIF columns
      if(td.previousElementSibling && td.previousElementSibling.textContent.includes('DIF')){
        td.classList.add(n>=10?'metric-diff-good':n>=0?'metric-diff-pos':n>-10?'metric-diff-warn':'metric-diff-bad');
      }
   });
 });
 ...
 document.querySelectorAll('thead th').forEach((th,idx)=>{
   ...
   document.querySelectorAll('tbody tr').forEach(r=>{
     ...
     if(!td || r.classList.contains('total-row')) return;
     const n=parseFloat(td.textContent.trim().replace('%','').replace(',',''));
     if(isNaN(n)) return;
     if(label.includes('PROD')){
       td.classList.add(n>=4?'metric-prod-high':n>=3?'metric-prod-good':n>=2.5?'metric-prod-mid':n>=2?'metric-prod-low':'metric-prod-bad');
     }
     if(label.includes('HC SIN')){
       td.classList.add(n<=2?'metric-hc-good':n<=5?'metric-hc-mid':'metric-hc-bad');
     }
     if(label.includes('% DIF')){
       td.classList.add(n>=10?'metric-diff-good':n>=0?'metric-diff-pos':n>-10?'metric-diff-warn':'metric-diff-bad');
     }
   });
 });
}
document.addEventListener('DOMContentLoaded',applyExecutiveColors);
