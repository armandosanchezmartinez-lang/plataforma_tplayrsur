...
foreach($distritos as $d){
    ...
    $stmt=mysqli_prepare($conexion,"SELECT COALESCE(SUM(forecast),0) total FROM metas_forecast_semanal WHERE anio=? AND semana=? AND CASE WHEN UPPER(TRIM(distrito)) LIKE '%COATZA%' AND UPPER(TRIM(distrito)) LIKE '%MINA%' THEN 'COATZA / MINA' ELSE UPPER(TRIM(distrito)) END=?");
    mysqli_stmt_bind_param($stmt,"iis",$anio,$semana,$d); mysqli_stmt_execute($stmt); $res=mysqli_stmt_get_result($stmt);
