...
function meta_mensual_estandar($distrito, $canal, $metas_estandar = []) {
    $d = normaliza_key($distrito);
    $c = normaliza_key($canal);

    if (isset($metas_estandar[$d.'|'.$c])) return (float)$metas_estandar[$d.'|'.$c];
    if (isset($metas_estandar[$d.'|*'])) return (float)$metas_estandar[$d.'|*'];
    if (isset($metas_estandar[$d])) return (float)$metas_estandar[$d];

    // Regla estándar solicitada: Villahermosa 26, resto de distritos 16.
    return (strpos($d, 'VILLAHERMOSA') !== false) ? 26.0 : 16.0;
}
...
// Tabla opcional recomendada: reai_metas_estandar(distrito, canal_venta, meta_mensual).
// Si no existe, se aplica la regla fija: Villahermosa=26 y resto=16.
$metas_estandar = [];
if (table_exists($conexion, 'reai_metas_estandar')) {
    $res_me = mysqli_query($conexion, "SELECT distrito, canal_venta, meta_mensual FROM reai_metas_estandar");
    while ($res_me && $row_me = mysqli_fetch_assoc($res_me)) {
        $dk = normaliza_key($row_me['distrito'] ?? '');
        $ck = normaliza_key($row_me['canal_venta'] ?? '');
        if ($dk !== '') {
            $metas_estandar[$dk . '|' . ($ck !== '' ? $ck : '*')] = (float)($row_me['meta_mensual'] ?? 0);
        }
    }
}
    ...
    <div class="table-card">
        <table>
            ...
            <?php foreach ($vendedores as $vend):
                ...
                // 2) Si no hay meta capturada, se usa meta estándar mensual: Villahermosa=26, resto=16
                //    o la tabla opcional reai_metas_estandar si existe.
                $meta_semanal_eo = (int)($st['meta_semanal_eo'] ?? 0);
                if ($meta_semanal_eo > 0) {
                    $meta_diaria = $meta_semanal_eo / max(1, $dias_habiles_semana_meta);
                } else {
                    $meta_mensual_std = meta_mensual_estandar($vend['distrito'] ?? '', $vend['canal_venta'] ?? '', $metas_estandar);
