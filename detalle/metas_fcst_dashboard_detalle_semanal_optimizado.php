...
$stmt=mysqli_prepare($conexion,"
    SELECT distrito, semana, COALESCE(SUM(meta),0) total
    FROM metas_instalacion_semanal
    WHERE anio = ?
      AND semana BETWEEN 1 AND ?
    GROUP BY distrito, semana
");
if($stmt){
    ...
    while($x=mysqli_fetch_assoc($res)){
        $dist=$x['distrito']; $sem=(int)$x['semana'];
...
$stmt=mysqli_prepare($conexion,"
    SELECT distrito, semana, COALESCE(SUM(forecast),0) total
...
$stmt=mysqli_prepare($conexion,"
    SELECT distrito, WEEK(fecha,1) semana, COUNT(cuenta) total
    FROM instalaciones
    WHERE YEAR(fecha) = ?
    ...
    GROUP BY distrito, WEEK(fecha,1)
");
if($stmt){
    mysqli_stmt_bind_param($stmt,"ii",$anio,$semana);
    mysqli_stmt_execute($stmt);
...
/* Calcula porcentajes del detalle */
