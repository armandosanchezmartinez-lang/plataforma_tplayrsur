...
$label_periodo_base = $periodo === 'mensual'
    ? $meses_es[$mes_base].' '.$anio_mes_base
    : 'Semana '.$semana_base;

$label_periodo_actual = $periodo === 'mensual'
    ? $meses_es[$mes_actual].' '.$anio_mes_actual
    : 'Semana '.$semana_actual;

$label_col_base = $periodo === 'mensual' ? strtoupper(substr($meses_es[$mes_base],0,3)) : 'SEM'.$semana_base;
$label_col_actual = $periodo === 'mensual' ? strtoupper(substr($meses_es[$mes_actual],0,3)) : 'SEM'.$semana_actual;

$cond_i_base = $periodo === 'mensual'
    ? "YEAR(i.fecha) = {$anio_mes_base} AND MONTH(i.fecha) = {$mes_base}"
    : "YEAR(i.fecha) = {$anio_base} AND WEEK(i.fecha,1) = {$semana_base}";

$cond_i_actual = $periodo === 'mensual'
    ? "YEAR(i.fecha) = {$anio_mes_actual} AND MONTH(i.fecha) = {$mes_actual}"
    : "YEAR(i.fecha) = {$anio_actual} AND WEEK(i.fecha,1) = {$semana_actual}";

$cond_ibase = $periodo === 'mensual'
    ? "YEAR(ibase.fecha) = {$anio_mes_base} AND MONTH(ibase.fecha) = {$mes_base}"
    : "YEAR(ibase.fecha) = {$anio_base} AND WEEK(ibase.fecha,1) = {$semana_base}";

$cond_iactual = $periodo === 'mensual'
    ? "YEAR(iactual.fecha) = {$anio_mes_actual} AND MONTH(iactual.fecha) = {$mes_actual}"
...
$subtitle = "Comparativo {$label_periodo_base} vs {$label_periodo_actual} · {$fecha_label} · " . ($roles_labels[$rol] ?? $rol);
