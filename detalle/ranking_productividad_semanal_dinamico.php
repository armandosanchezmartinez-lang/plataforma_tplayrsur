...
function contar_dias_habiles_semana_seleccionada($conexion, $anio, $semana, $dias_iso) {
    ...
    foreach ($dias_iso as $dia_iso) {
        $dia_iso = (int)$dia_iso;
        if ($dia_iso < 1 || $dia_iso > 6) continue; // domingo fuera de operación
...
function previous_iso_week($anio, $semana) {
    ...
    $prev_anio = $anio - 1;
    $last = (int)(new DateTime($prev_anio.'-12-28'))->format('W');
    return [$prev_anio, $last];
}
...
// Semana operativa máxima: permite ver avance semanal aunque aún no exista plantilla HC.
...
// Si por URL intentan navegar más allá de la última semana con instalaciones,
// regresar a la semana operativa máxima.
if (
...
// En semana corriente puede no existir plantilla HC.
// Si no existe HC de la semana seleccionada, usamos la semana anterior disponible como proxy.
...
$dias_semana_mysql = array_map(function($d) { return (int)$d + 1; }, $dias_semana_seleccionados); // ISO 1=LUN => MySQL DAYOFWEEK 2=LUN
...
.weekday-day:hover{background:#e8f0fe}
...
<script>
(function(){
    const dropdown = document.querySelector('.week-range-dropdown');
