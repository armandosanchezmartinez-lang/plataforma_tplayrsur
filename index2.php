...
function tx_segmento_instalacion_expr_dashboard($conexion, $alias = '') {
    /*
    |--------------------------------------------------------------------------
    | TOTALXPEDIENT - RESIDENCIAL / NEGOCIOS
    |--------------------------------------------------------------------------
    | Misma lógica base de Ranking Vendedor:
    | el paquete instalado se clasifica contra catalogo_paquetes.
    |
    | Matching:
    |   instalaciones.plan / nombre_plan / paquete / nombre_paquete
    |   contra catalogo_paquetes.nombre_plan.
    |
    | Negocios:
    |   si el valor clasificador contiene NEGOC / BUSINESS / EMPRES / PYME / SME.
    | Resto:
    |   Residencial.
    |--------------------------------------------------------------------------
    */
    ...
    // Respaldo defensivo si falta catálogo o no empata el plan.
    $inst_cols = [
        'segmento','tipo_segmento','tipo_cliente','tipo_servicio','tipo_venta',
...
function tx_segmento_venta_expr_dashboard($conexion, $alias = '') {
    ...
    |
    | Si no hay match exacto o falta el catálogo, se clasifica como RESIDENCIAL.
    |--------------------------------------------------------------------------
    */
    $prefix = $alias !== '' ? "`" . str_replace("`", "", $alias) . "`." : "";
    ...
    return "COALESCE((
        SELECT CASE
            WHEN UPPER(TRIM(cp.`tipo`)) = 'NEGOCIOS' THEN 'NEGOCIOS'
            ELSE 'RESIDENCIAL'
        END
        FROM catalogo_paquetes cp
...
function tx_mix_res_neg_where_dashboard($conexion, $where_sql) {
    ...
    if ($total <= 0) return ['total'=>0,'res_pct'=>null,'neg_pct'=>null];

    $neg_pct = (int)round(($neg / $total) * 100);
    $res_pct = max(0, 100 - $neg_pct);
    return ['total'=>$total, 'res_pct'=>$res_pct, 'neg_pct'=>$neg_pct];
}
...
function tx_mix_res_neg_folios_dashboard($conexion, $folios, $mes, $anio, $cond_dia_fecha) {
    if (empty($folios)) return ['total'=>0,'res_pct'=>null,'neg_pct'=>null];
...
function tx_mix_res_neg_canal_dashboard($conexion, $canal, $mes, $anio, $cond_dia_fecha, $scope_sql = '') {
    ...
    $regla = $mapa[$canal] ?? [];
    $canales_real = $regla['real'] ?? [];
    if (empty($canales_real)) return ['total'=>0,'res_pct'=>null,'neg_pct'=>null];
...
if ($mostrar_cumplimiento_canal) {
    ...
    foreach ($canales_cumplimiento as $canal) {
        ...
        $mix_res_neg_canal = tx_mix_res_neg_canal_dashboard($conexion, $canal, $mes_actual, $anio_query, $cond_dia_fecha, $scope_canal_sql);
        $cumplimiento_canal_items[] = [
            'nombre' => $canal,
            'real' => $real_canal,
            'meta' => $meta_canal,
...
while($row = $res_vo ? mysqli_fetch_assoc($res_vo) : null) {
    ...
    $idx = array_search($label_mes, $meses_labels);
    if($idx !== false) {
        $oferta_norm = strtoupper(trim((string)$row['oferta']));
        $oferta = ($oferta_norm === 'NEGOCIOS') ? 'Negocios' : 'Residencial';
        $datos_vent_oferta_stacked[$oferta][$idx] = (int)$row['total'];
    }
}


function tx_debug_resneg_ventas_dashboard($conexion, $fecha_inicio_evolucion, $cond_dia_evolucion_fecha_cierre, $scope_sql_clause) {
    $html = '';

    $sql_resumen = "
        SELECT
            YEAR(v.fecha_cierre) AS anio,
            MONTH(v.fecha_cierre) AS mes,
            CASE
                WHEN cp.nombre_plan IS NULL THEN 'SIN MATCH'
                WHEN UPPER(TRIM(cp.tipo)) = 'NEGOCIOS' THEN 'NEGOCIOS'
                ELSE 'RESIDENCIAL'
            END AS oferta,
            COUNT(*) AS total
        ...
        LEFT JOIN catalogo_paquetes cp
          ON UPPER(TRIM(v.nombre_plan)) = UPPER(TRIM(cp.nombre_plan))
        WHERE v.fecha_cierre >= '$fecha_inicio_evolucion'
          $cond_dia_evolucion_fecha_cierre
          $scope_sql_clause
        GROUP BY anio, mes, oferta
        ORDER BY anio, mes, oferta
    ";

    $rows_resumen = [];
    $r = mysqli_query($conexion, $sql_resumen);
    while ($r && ($row = mysqli_fetch_assoc($r))) $rows_resumen[] = $row;

    $sql_sample = "
        SELECT
            v.nombre_plan AS plan_ventas,
            cp.nombre_plan AS plan_catalogo,
            cp.tipo AS tipo_catalogo,
            COUNT(*) AS total
        FROM ventas v
        LEFT JOIN catalogo_paquetes cp
          ON UPPER(TRIM(v.nombre_plan)) = UPPER(TRIM(cp.nombre_plan))
        WHERE v.fecha_cierre >= '$fecha_inicio_evolucion'
          $cond_dia_evolucion_fecha_cierre
          $scope_sql_clause
        GROUP BY v.nombre_plan, cp.nombre_plan, cp.tipo
        ORDER BY total DESC
        LIMIT 50
    ";

    $rows_sample = [];
    $s = mysqli_query($conexion, $sql_sample);
    while ($s && ($row = mysqli_fetch_assoc($s))) $rows_sample[] = $row;

    ob_start();
    ?>
    <div class="evo-card" style="border:2px dashed #ff006c; margin:18px 0;">
        <div class="chart-title">DEBUG Ventas por oferta RES-NEG</div>
        <div style="font-size:12px;color:#334155;margin-bottom:12px;">
            Match usado: <strong>ventas.nombre_plan = catalogo_paquetes.nombre_plan</strong> · Clasificación: <strong>catalogo_paquetes.tipo</strong>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1.4fr;gap:18px;">
            <div>
                <div class="evo-sub">Resumen por mes</div>
                <table class="participacion-table">
                    <thead><tr><th>Año</th><th>Mes</th><th>Oferta</th><th>Total</th></tr></thead>
                    <tbody>
                    <?php foreach ($rows_resumen as $rr): ?>
                        <tr>
                            <td><?= htmlspecialchars($rr['anio']) ?></td>
                            <td><?= htmlspecialchars($rr['mes']) ?></td>
                            <td><?= htmlspecialchars($rr['oferta']) ?></td>
                            <td><?= number_format((int)$rr['total']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <div>
                <div class="evo-sub">Top planes ventas vs catálogo</div>
                <table class="participacion-table">
                    <thead><tr><th>ventas.nombre_plan</th><th>catalogo.nombre_plan</th><th>catalogo.tipo</th><th>Total</th></tr></thead>
                    <tbody>
                    <?php foreach ($rows_sample as $rs): ?>
                        <tr>
                            <td><?= htmlspecialchars($rs['plan_ventas'] ?? '') ?></td>
                            <td><?= htmlspecialchars($rs['plan_catalogo'] ?? 'SIN MATCH') ?></td>
                            <td><?= htmlspecialchars($rs['tipo_catalogo'] ?? '') ?></td>
                            <td><?= number_format((int)$rs['total']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <details style="margin-top:12px;">
            <summary style="cursor:pointer;font-weight:800;">SQL resumen</summary>
            <pre style="white-space:pre-wrap;background:#0f172a;color:#e5e7eb;padding:12px;border-radius:12px;font-size:11px;overflow:auto;"><?= htmlspecialchars($sql_resumen) ?></pre>
        </details>
    </div>
    <?php
    return ob_get_clean();
...
$scope_debug_resneg_ventas = '';
if ($rol_consulta !== 'admin' && $scope_filtrar_por_distrito) {
    $scope_debug_resneg_ventas = " AND v.distrito IN ($scope_distritos_sql)";
} elseif ($rol_consulta !== 'admin' && $por_distrito) {
    $scope_debug_resneg_ventas = " AND v.distrito IN ($distritos_sql)";
} elseif ($rol_consulta !== 'admin' && !empty($folio_ids)) {
    $ph_dbg = implode("','", array_values($folio_ids));
    $scope_debug_resneg_ventas = " AND v.folio_empleado IN ('$ph_dbg')";
}

$debug_resneg_ventas_html = '';
if (isset($_GET['debug_resneg']) && $_GET['debug_resneg'] === '1') {
    $debug_resneg_ventas_html = tx_debug_resneg_ventas_dashboard(
        $conexion,
        $fecha_inicio_evolucion,
        $cond_dia_evolucion_fecha_cierre,
        $scope_debug_resneg_ventas
    );
}
...
<head>
    ...
    <style>
        ...
        .cumplimiento-track{ position:relative; }
        .cumplimiento-fill{ position:relative; overflow:hidden; }
...
<?php if (!empty($debug_resneg_ventas_html)) echo $debug_resneg_ventas_html; ?>
<script>
/* ── DROPDOWN USER BADGE ── */
